...
def upgrade() -> None:
    ...
    # 先创建 PostgreSQL ENUM 类型（使用 DO block 避免 CREATE TYPE IF NOT EXISTS 语法不支持）
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'studentlevel') THEN
                CREATE TYPE studentlevel AS ENUM ('excellent', 'average', 'basic');
            END IF;
        END $$
    """)
    op.execute("""
        DO $$ BEGIN
            IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'studentattitude') THEN
                CREATE TYPE studentattitude AS ENUM ('active', 'neutral', 'passive');
            END IF;
        END $$
    """)
    ...
    sa.Column('level', ENUM('excellent', 'average', 'basic', name='studentlevel', create_type=False), nullable=False),
    sa.Column('attitude', ENUM('active', 'neutral', 'passive', name='studentattitude', create_type=False), nullable=False),
