...
- `test_clear_message_history_empties_list` - 清空消息列表
- `test_clear_message_history_resets_last_summary_update` - 重置摘要更新计数器
- `test_clear_message_history_preserves_checkpoint_summaries` - 不影响检查点摘要

#### TestSessionMemoryGetFullContext (5 个测试)
- `test_get_full_context_includes_topic` - 包含教学主题
- `test_get_full_context_includes_checkpoint_summaries` - 包含检查点摘要
- `test_get_full_context_includes_teaching_summary` - 包含教学摘要
- `test_get_full_context_includes_recent_messages` - 包含最近消息
- `test_get_full_context_empty_state` - 空状态
...
- `test_init_checkpoint_summaries_default` - checkpoint_summaries 默认为空列表

#### TestMemoryManagerSummarizeCheckpoint (7 个测试)
- `test_summarize_checkpoint_appends_summary` - 摘要追加到 checkpoint_summaries
- `test_summarize_checkpoint_clears_message_history` - 清空消息历史
- `test_summarize_checkpoint_resets_last_summary_update` - 重置计数器
...
- `test_summarize_checkpoint_summary_failure_still_clears_history` - 异常时仍清除
- `test_summarize_checkpoint_accumulates_multiple_summaries` - 多次调用累积
...
- `test_end_feedback_uses_full_context_not_agent_context` - 使用检查点摘要上下文
- `test_end_feedback_uses_lightweight_context` - 不包含大量历史消息
- `test_build_end_feedback_context` - 方法存在且返回正确格式
- `test_end_feedback_records_message_to_history` - 回归测试：记录 END_FEEDBACK 消息
